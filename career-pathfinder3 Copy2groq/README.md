...
no AI call. Gemini is only used as a fallback, for open-ended follow-up questions that don't match one of
those predefined patterns. Everything is persisted per logged-in user in PostgreSQL.

**Stack:** React (Vite) · FastAPI · PostgreSQL (SQLAlchemy) · Google Gemini (follow-up chat fallback only)

### Why the recommendation step no longer calls Gemini

Career recommendation is the first, highest-traffic step of the app — every single onboarding run used
to make a Gemini request, which burned through the free-tier quota fast. That step is now handled by
...
careers" below. Gemini now only gets called from the follow-up chat (`/api/chat`), and even then only
...
answerer, so a full run of onboarding + tapping every tag question costs **zero** Gemini requests.
...
│   │       └── gemini_service.py     # Gemini prompt building + calls, fallback-only for /api/chat
│   ├── requirements.txt
│   └── .env.example
└── frontend/
    ├── src/
...
- A Gemini API key from https://aistudio.google.com/apikey — **optional now.** Recommendations and the
...
GEMINI_API_KEY=your_real_gemini_api_key   # optional — only used as a fallback for open-ended chat
...
3. **Tag questions** — the result screen shows five predefined chips (`TAG_QUESTIONS` in
   ...
   from the stored recommendation — no Gemini call.
4. **Follow-up chat** posts to `/api/chat` with the `recommendation_id`. The backend first tries
   ...
   `gemini_service.get_followup_reply`, loading the profile + recommendation + prior chat history as
...
1. Provision their managed Postgres, copy the connection string into `DATABASE_URL` on the backend
   ...
   frontend URL, and `GEMINI_API_KEY` if you want the chat fallback to work).
...
- No rate limiting on the Gemini-calling fallback path — worth adding before exposing this publicly, since
